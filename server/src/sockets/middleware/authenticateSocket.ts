 ...
 * Validates the JWT token sent in the socket handshake (via HttpOnly cookie)
...
export const authenticateSocket = async (
  ...
  try {
    // 1️⃣ Extract cookies from handshake headers
    const cookiesHeader = socket.handshake.headers.cookie;
    if (!cookiesHeader) {
      socket.emit("connection:denied", {
        success: false,
        reason: "no_cookie",
        message: "No cookies sent with the connection",
      });
      return next(new Error("Connection denied: No cookies sent"));
    }

    const cookies = cookie.parse(cookiesHeader);
    const token = cookies["Nexion-token"];
    ...
    // 5️⃣ Attach user info to socket
    socket.user = {
