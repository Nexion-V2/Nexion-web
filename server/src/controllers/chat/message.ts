...
export const getMessages = async (req: Request<any, any, any, GetMessagesQuery>, res: Response) => {
    ...
    console.log("limit: ",limit, " skip: ",req.query.skip);
    ...
    try {
        ...
        */
        ...
        const messages: FlattenMaps<IMessage>[] = await Message.find({ conversationId })
        ...
        res.status(200).json({ 
            success: true, 
            messages: messages, // Reverse to chronological order
            hasMore,
            total: totalMessages
        });
