...
export default function ReplyView({ replyToId }: { replyToId: string }) {
  ...
  const replyingTo = allMessages.find((msg) => msg._id === replyToId);
  if (!replyingTo)
    return (
      <span className="text-sm text-gray-400 italic">Message removed</span>
    );
  ...
  const handleClick = () => {
    ...
    setTimeout(() => {
      messageElement.classList.remove("ease-in-out", "opacity-50", "scale-102");
    }, 2000);
  };
  ...
  return (
    ...
    >
      ...
      {/* Text content */}
      {replyingTo.content?.text && <TextCard msg={replyingTo.content as any} />}

      {/* Images */}
      {attachments.some((att) => att.type === "image") && (
        <ImageCard msg={replyingTo.content as any} />
      )}

      {/* Audio */}
      {attachments.some((att) => att.type === "audio") && (
        <AudioCard msg={replyingTo.content as any} />
      )}
